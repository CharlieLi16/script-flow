...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { provider, model, prompt, size, referenceData } = body;
    if (!provider || !prompt) {
    ...
    return jsonResponse(res, 200, encodeGenerationResponse(result));
  } catch (err) {
    return jsonResponse(res, err.status || 500, { error: err.message || 'Generation failed' });
  }
}
